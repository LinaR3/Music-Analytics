...
const customStyles = `
  /* IMPORTACIÓN DE FUENTES CLAVE */
  @import url('https://fonts.googleapis.com/css2?family=Poiret+One&family=Montserrat:wght@400;600;700&display=swap');

  /* VARIABLES CSS GLOBALES */
  :root {
    --primary-color-dark: #0077B6; /* Azul primario oscuro */
    --accent-color-dark: #00FFFF; /* Cian acento oscuro */
    --bg-color-dark: #0b132b; /* Azul marino muy oscuro para fondo */
    --card-bg-color-dark: #1c2541; /* Azul marino oscuro para tarjetas */
    --text-color-dark: #E5E7EB; /* Gris claro para texto */
    --text-muted-dark: #a0aec0; /* Gris azulado para texto muted */
    --glass-bg-dark: rgba(28, 37, 65, 0.6); /* Fondo glass oscuro (card bg con alpha) */
    --glass-border-dark: rgba(0, 255, 255, 0.2); /* Borde glass oscuro (cian alpha) */
    --glass-hover-bg-dark: rgba(0, 255, 255, 0.1); /* Hover glass oscuro (cian alpha) */
    --btn-text-dark: #E5E7EB;
    --btn-text-hover-dark: #00FFFF;

    --primary-color-light: #005f99; /* Azul primario claro más oscuro */
    --accent-color-light: #0077B6; /* Azul acento claro */
    --bg-color-light: #f0f4f8; /* Gris muy claro azulado para fondo */
    --card-bg-color-light: #FFFFFF; /* Blanco para tarjetas */
    --text-color-light: #1F2937; /* Gris oscuro para texto */
    --text-muted-light: #4b5563; /* Gris medio para texto muted */
    --glass-bg-light: rgba(255, 255, 255, 0.7); /* Fondo glass claro */
    --glass-border-light: rgba(0, 119, 182, 0.3); /* Borde glass claro (azul alpha) */
    --glass-hover-bg-light: rgba(0, 119, 182, 0.1); /* Hover glass claro (azul alpha) */
    --btn-text-light: #1F2937;
    --btn-text-hover-light: #005f99;
  }

  .dark { /* Variables modo oscuro */
  ...
  .light { /* Variables modo claro */
    ...
    --glass-hover-bg: var(--glass-hover-bg-light);
  ...
  .font-title-display { font-family: 'Poiret One', cursive; font-weight: 400 !important; }
  .font-body-sintony { font-family: 'Montserrat', sans-serif; font-weight: 400 !important; }
  .text-gradient-accent { background: linear-gradient(to right, var(--primary-color, #0077B6), var(--accent-color, #00FFFF)); -webkit-background-clip: text; background-clip: text; color: transparent; }
  .dark .text-gradient-accent { background: linear-gradient(to right, #90E0EF, #00FFFF); -webkit-background-clip: text; background-clip: text; color: transparent; }

  /* Recharts Tooltip */
  .recharts-tooltip-wrapper { z-index: 1000 !important; }
  .recharts-default-tooltip { background-color: var(--card-bg-color) !important; border: 1px solid var(--glass-border) !important; border-radius: 8px !important; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2) !important; padding: 10px 15px !important; }
  .recharts-tooltip-label { color: var(--text-color) !important; font-weight: 600; margin-bottom: 8px !important; padding-bottom: 8px !important; border-bottom: 1px solid var(--glass-border) !important; font-family: 'Montserrat', sans-serif; }
  .recharts-tooltip-item-list { color: var(--text-muted) !important; font-family: 'Montserrat', sans-serif; font-size: 0.85rem; }
  .recharts-tooltip-item { margin-bottom: 5px !important; }
  .recharts-tooltip-item-name, .recharts-tooltip-item-separator { color: var(--text-muted) !important; }
  .recharts-tooltip-item-value { color: var(--text-color) !important; font-weight: 600; margin-left: 8px; }

  /* Scrollbar */
  .custom-scrollbar::-webkit-scrollbar { width: 6px; height: 6px; }
  .custom-scrollbar::-webkit-scrollbar-thumb { background-color: var(--accent-color); border-radius: 3px; opacity: 0.6; }
  .custom-scrollbar::-webkit-scrollbar-track { background: var(--card-bg-color); opacity: 0.3; }

  /* Botón Glass */
  .btn-glass { background-color: var(--glass-bg); backdrop-filter: blur(5px); -webkit-backdrop-filter: blur(5px); border: 1px solid var(--glass-border); color: var(--btn-text); transition: all 300ms ease; border-radius: 0.5rem; padding: 0.6rem 1.2rem; }
  .btn-glass:hover:not(:disabled) { background-color: var(--glass-hover-bg); color: var(--btn-text-hover); border-color: var(--accent-color); transform: translateY(-2px); box-shadow: 0 4px 10px rgba(0, 255, 255, 0.1); }
  .dark .btn-glass:hover:not(:disabled) { box-shadow: 0 4px 15px rgba(0, 255, 255, 0.15); }
  ...
  /* Animación para el carrusel (fade) */
  @keyframes fadeIn {
    from { opacity: 0.3; }
    to { opacity: 1; }
  }
  ...
  .dark .hover-shadow-lg:hover { box-shadow: 0 10px 15px -3px rgba(0, 255, 255, 0.07), 0 4px 6px -2px rgba(0, 255, 255, 0.04); }
  ...
  .recharts-bar-cursor-pointer:hover, .recharts-cell-cursor-pointer:hover { opacity: 0.8; cursor: pointer; }
  input[type=range] { /* Estilos input range */ -webkit-appearance: none; width: 100%; height: 8px; background: var(--text-muted); outline: none; opacity: 0.7; transition: opacity .2s; border-radius: 4px; margin-top: 5px; margin-bottom: 5px; }
  input[type=range]:hover { opacity: 1; }
  input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 20px; height: 20px; background: var(--accent-color); cursor: pointer; border-radius: 50%; box-shadow: 0 0 5px var(--accent-color); }
  input[type=range]::-moz-range-thumb { width: 20px; height: 20px; background: var(--accent-color); cursor: pointer; border-radius: 50%; border: none; box-shadow: 0 0 5px var(--accent-color); }

  /* Hero Background Pattern (Eliminado) */
   .hero-pattern { }
   .light .hero-pattern { }

   /* Clip-path para Hero (Eliminado) */
   .clip-angled { }
   @media (max-width: 767px) { .clip-angled { clip-path: none; } }

  /* Clase Prose */
  .prose { max-width: none; } .prose code::before, .prose code::after { content: "" !important; } .prose code { font-family: monospace; background-color: rgba(128, 128, 128, 0.1); padding: 0.1em 0.3em; border-radius: 4px; font-size: 0.9em; } .prose pre { background-color: rgba(128, 128, 128, 0.1); padding: 1em; border-radius: 6px; overflow-x: auto; } .prose pre code { background-color: transparent; padding: 0; font-size: 0.85em; } .prose ul { list-style: disc; margin-left: 1.5em; padding-left: 0; } .prose li { margin-bottom: 0.25em; } .prose h1, .prose h2, .prose h3 { margin-bottom: 0.5em; margin-top: 1em; }

  /* Tooltip personalizado para gráfico Pie */
    .custom-pie-tooltip { background-color: var(--card-bg-color); border: 1px solid var(--glass-border); border-radius: 6px; padding: 8px 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.2); font-size: 0.8rem; color: var(--text-color); }
    .custom-pie-tooltip .label { font-weight: bold; color: var(--accent-color); }
...
const HomeCarousel = memo(({ isDark, setActiveTab }) => {
    ...
    const slides = [
        {
            ...
            imageSrc: 'image_5ceaea.jpg', // Tu imagen principal
    ...
    // const goToPrev = useCallback(() => { ... }); // ELIMINADO
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`flex items-center gap-1 sm:gap-1.5 px-3 py-1.5 rounded-md text-sm transition-colors duration-200 font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[var(--bg-color)] focus:ring-[var(--accent-color)] ${
                activeTab === item.id
                  ? 'bg-[var(--accent-color)]/15 text-[var(--accent-color)] shadow-inner shadow-[var(--accent-color)]/10'
